...
workbook = xlrd.open_workbook('E:/rssi_generator/datasets/Alice_Praproses.xls', on_demand=True)
...
book.save('E:/rssi_generator/datasets/hasilkuantisasiAlice.xls')
book.save(TemporaryFile())
end_jana=time.time()
time_jana=end_jana-start_jana
kgrkuan=len(hasilakhir)*(end_jana-start_jana)
#korelasi1=np.corrcoef(awalalice,awalbob)
#print("Korelasi pra Proses : %f" %korelasi1[0][1])
print('KGR = %f'%kgrkuan)
print('waktu komputasi kuantisasi = {}seconds'.format(end_jana-start_jana))
socketsend('hasilkuantisasiAlice.xls', init_socket())
socketrecv()
