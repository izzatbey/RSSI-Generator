...
samping=int(len(rss_alice)/bawah) #jumlah data yang diprocess

#-----reshape
alice=np.array(rss_alice)
rssival_alice=alice.reshape(samping,bawah).T
pjg_alice_sesudah = len(rssival_alice)
...
for i in range(0,25):
    ...
    for j in range (0,128):
        nilaibaris=(nilaibaris+rssival_alice[j][i])
...
for i in range(0,25):
    ...
    hasil_awalb21=[]
    for j in range (0,128):
        nilaibaris=(data_x1[j]*rssival_alice[j][i])
...
for i in range(0,25):
    ...
    hasil_awalb31=[]
    for j in range (0,128):
        nilaibaris=((data_x1[j]**2)*rssival_alice[j][i])
...
np.savetxt(os.path.join(args.destination, "reg_alice1.csv"), hasilgauss, delimiter=',')
end1 = time.time()
